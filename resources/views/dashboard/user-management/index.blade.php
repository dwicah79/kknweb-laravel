...
@section('konten')
    ...
    <div class="w-full flex justify-between items-center bg-gradient-primmary px-5 py-5 mb-2">
        <h1 class="font-extrabold uppercase text-white text-xl">USER MANAGEMENT</h1>
        <a href="{{ route('user.create') }}" class="bg-white rounded-lg w-fit whitespace-nowrap p-2 hover:bg-slate-100"><i
            ...
            <table class="w-full bg-gray-50 border border-gray-200 divide-y divide-gray-300 rounded-lg">
                <thead class="bg-blue-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700">Nama User</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700">Email</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700">Hak Akses</th>
                ...
                <tbody class="divide-y divide-gray-300">
                    @foreach ($data as $item)
                        <tr class="bg-white hover:bg-gray-100">
                            ...
                            <td class="px-4 py-4 text-sm text-gray-800">{{ $item->email }}</td>
                            <td class="px-4 py-4 text-sm text-gray-800">
                                @if ($item->roles->first()?->name == 'Super-Admin')
                                    <span class="badge-red">Super Admin</span>
                                @elseif ($item->roles->first()?->name == 'Pengurus-Desa')
                                    <span class="badge-green">Pengurus Desa</span>
                                @elseif ($item->roles->first()?->name == 'Pengurus-Pemuda')
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('user.edit', $item->id) }}" class="text-blue-300 hover:text-blue-400">
                                        <i class="fa-solid fa-eye"></i>
                                        </svg>
                                    </a>
                                    <form id="delete-form-{{ $item->id }}"
                                        action="{{ route('user.destroy', $item->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
