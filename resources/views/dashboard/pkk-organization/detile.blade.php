...
@section('konten')
    <x-breadcrumb :links="[
        ...
        ['name' => 'STRUKTUR PKK', 'url' => route('pkk.index')],
        ['name' => 'EDIT DATA STRUKTUR PKK', 'url' => '#'],
    ]" />

    <div class="rounded-lg bg-white p-10">
        <x-alert type="error" />
        <form action="{{ route('pkk.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="flex flex-wrap w-full">
                ...
                <div class="mb-5 w-full md:w-1/2 px-5">
                    <label for="telp" class="block mb-2 text-sm font-medium text-gray-900 ">No Telp. <span
                            class="text-red-500">*</span></label>
                    <input type="number" id="telp" name="telp" value="{{ $data->telp }}"
                ...
                <div class="mb-5 px-5 w-full md:w-1/2">
                    ...
                    <select name="position" id="position" required
                        ...
                        @foreach ($position as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $data->job_title_id ? 'selected' : '' }}>
                                {{ $item->job_title }}
                            </option>
                        @endforeach
                    </select>
